...
def get_song_data(mode):
    song_tds = [td for td in
                      soup.find_all('td',
                                    class_=f"chartlist-{mode}")]  #A list of all td elements with this class
    ...
    for a in initial_a_list:
        ...
        counter += 1
        if counter > int(get_config_data("number_of_recent_tracks")):
            link = str(a)
            link = link.replace("<", ">")
            split_link = link.split(">")
    ...
    if mode == "name": #Only executes when retrieving songs not their artists
        play_spans = [span for span in
                      soup.find_all('span',
                                    class_="chartlist-count-bar-value")]  #A list of all span elements with this class
    ...
    if mode == "name":
        for span in play_spans:
            ...
            split_playcount = playcount.split(">")
            plays = split_playcount[2].replace("\n", "")
            while plays.startswith(" "):
                plays = plays.lstrip(" ")
            song_plays.append(plays)

    songs_and_plays = []

    songs = {}
    if len(song_plays) == len(song_list):
        for i in range(len(song_list)):
            songs_and_plays.append(song_list[i])
            songs_and_plays.append(song_plays[i])


    for i in range(0, len(songs_and_plays), 2):
        songs[songs_and_plays[i]] = songs_and_plays[i + 1]
...
songs = get_song_data("name")
top_song_artists = get_song_data("artist")
...
with open('Last.fm.txt', 'w') as f:  #Writes data to txt file
    ...
    counter = 0
    for key in songs:
        print(key, f"by {top_song_artists[counter]}",' : ', songs[key], file=f)
        counter += 1
    print("}\n", file=f)
